...
# =========== LEXIQUE ===========
LEXICON = {'lithology': ['mort-terrain', 'remblais', 'silt', 'grès', 'siltstone', 'shale', 'conglomérat', 'mudstone', 'calcaire', 'dolomite', 'sel', 'halite', 'anhydrite', 'gypse', 'sylvite', 'argile', 'boue', 'limon', 'sable', 'gravier', 'blocs rocheux'], 'modifier': ['silteu(?:x|se)', 'sableu(?:x|se)', 'argileu(?:x|se)', 'shisteu(?:x|se)', 'boueu(?:x|se)', 'caillouteu(?:x|se)', 'graveleu(?:x|se)', 'bitumineu(?:x|se)'], 'quantity': ['strie(?:s)?', 'veine(?:s)?', 'filet(?:s)?', 'intercalé(?:s|e|es)?', 'intercalation(?:s)?', 'beaucoup', 'plusieurs', 'interstratifié(?:s|e|es)?', 'tacheté(?:s)?', 'bande(?:s)?', 'fragment(?:s)?', 'impurité(?:es)', 'abondant(?:e|es)', 'mineur', 'quelques', 'rare', 'flocon(?:s)?', 'trace(?:s)', '[-.\\d]+%', '[-.\\d]+pc', '[-.\\d]+pourcent'], 'grainsize': ['vf(?:-)?', 'f(?:-)?', 'm(?:-)?', 'c(?:-)?', 'vc', 'très fin(?:e à)?', 'fin(?:e|s|es à)?', 'moyen(?:ne à)?', 'grossi(?:er|ers|ère|ères à)?', 'très grossi(?:er|ère)', 't fin(?:e|s|es à)?', 'moy(?: à)?', 'moy.(?: à)?', 't grossier(?:s)?', 'granules?', 'cailloux?', 'galets?', 'blocs rocheux?'], 'colour': ['rouge(?:âtre)?', 'gris(?:e|âtre)?', 'noir(?:e|âtre)?', 'blanc(?:he|hâtre)', 'bleu(?:e|âtre)', 'rose(?:âtre)', 'jaun(?:e|âtre)?', 'ver(?:t|dâtre)?', 'marron?', 'clair', 'sombre'], 'synonyms': {'mort-terrain': ['terres'], 'Anhydrite': ['Gypse'], 'Sel': ['Halite', 'Sylvite']}, 'splitters': [' avec ', ' de ', ' cont(?:ient|enant) ', '\\. '], 'parts_of_speech': {'noun': ['lithologie'], 'adjective': ['couleur', 'taille_grain', 'modifier'], 'subordinate': ['quantité']}}
