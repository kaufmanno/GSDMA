...
from utils.lexicon.lexicon_memoris import pollutants_memoris, soil_standards


def get_contam_level_from_df(df, verbose=False):
    abbr_keys = list(pollutants_memoris.abbreviations.keys())
    abbr_values = list(pollutants_memoris.abbreviations.values())
    pollutants = [p for p in df.columns if p in pollutants_memoris.abbreviations.keys() or p in pollutants_memoris.abbreviations.values()]
    ...
    for i in df.index:
        ...
        for c in df.columns:
            ...
            if c in pollutants:
                val = df.loc[i, c]
                if c in abbr_keys:
                    pol_name = pollutants_memoris.abbreviations[c]
                elif c in abbr_values:
                    pol_name = c
                else:
                    raise (NameError('Pollutant not found in lexicon!'))

                if pol_name in soil_standards['pollutants'].keys():
                    d = soil_standards['pollutants'][pol_name]
...
def get_contam_level_from_value(value, pollutant, verbose=False):
    ...
    if pollutant in abbr_keys:
        pol_name = pollutants_memoris.abbreviations[pollutant].lower()
    elif pollutant in abbr_values:
...
def striplog_from_dataframe(df, bh_name, attributes, symbols=None, intv_top=None,
    ...
    pollutants = list(pollutants_memoris.abbreviations.keys()) + \
                 list(pollutants_memoris.abbreviations.values())
        ...
        if bh_id not in bh_list:
            ...
            for j in tmp.index:
                ...
                for attrib in attributes:
                    ...
                    if attrib in pollutants and not isinstance(val, str):
